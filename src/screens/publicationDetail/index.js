...
const PublicationDetail = ({ navigation, route }) => {
    ...
    const photoProfile = publication?.userInfoData.profileImage
        ? publication?.userInfoData.profileImage
    ...
    return (
        <View style={styles.container}>
            <View style={width > 660 ? styles.userInfoContainerTablet : styles.userInfoContainer}>
                <Image
                    style={width > 660 ? styles.userImageTablet : styles.userImage}
                    source={{ uri: userInfo?.localId === publication?.localId ? userInfo?.profileImage : photoProfile }}
                />
                <View style={width > 660 ? styles.userContainerTablet : styles.userContainer}>
                    <Text style={width > 660 ? styles.userNameTablet : styles.userName}>
                        {publication.userInfoData.username}
                    </Text>
            ...
            {userCommerInfo ? (
                <View>
                    ...
                    <View style={width > 660 ? styles.commentButtonContainerTablet : styles.commentButtonContainer}>
                        <TouchableOpacity
                            style={[
                                width > 660 ? styles.commentButtonTablet : styles.commentButton,
                                { opacity: isButtonDisabled ? 0.5 : 1 },
                            ]}
                            onPress={onHandlerPublicationPublicate}
                            disabled={isButtonDisabled}
                        >
                            <Text style={width > 660 ? styles.commentButtonTextTablet : styles.commentButtonText}>
                                Create Answer
                            </Text>
                        </TouchableOpacity>
                    </View>
            ...
            {loading ? (
                ...
                <FlatList
                    data={Object.values(
                        combinedPublicationData.find((item) => item.id === publicationId)?.comments || {}
                    ).slice().reverse()}
                    ...
                    renderItem={({ item }) => (
                        <View style={width > 660 ? styles.commentpublicationTablet : styles.commentpublication}>
                            ...
                            <View style={width > 660 ? styles.commentpublicationInfoTablet : styles.commentpublicationInfo}>
                                ...
                                <Text style={width > 660 ? styles.commentpublicacionTextTablet : styles.commentpublicacionText}>
                                    {item?.commentsText}
                                </Text>
                            </View>
                            {localId === item.userCommerInfo?.localId ? (
                                <TouchableOpacity
                                    style={width > 660 ? styles.commentdeletePublicationTablet : styles.commentdeletePublication}
                                    onPress={() => handleDeleteComment(item.id)}
                                >
                                    <MaterialIcons
                                        name='delete'
                                        size={width > 660 ? 28 : 18}
